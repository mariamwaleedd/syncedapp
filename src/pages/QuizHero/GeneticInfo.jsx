...
const GeneticInfo = () => {
  ...
  const handleContinue = async () => {
    ...
    await supabase.from('application_healthId').update({ blood_type: selectedBlood }).eq('id', id);
    navigate('/emergencycontacts');
  };

  return (
    <div className="gi-screen">
      <div className="gi-bg-gradient"></div><div className="gi-bg-lines"></div>
      <div className="gi-content">
        ...
        <div className="gi-hero">
          ...
          <h1 className="gi-title">Genetic Information</h1><p className="gi-subtitle">Important for compatibility</p>
        </div>
        <div className="gi-main-card">
          <div className="gi-card-header"><Droplets size={18} color="#FF5252" /><span>Blood Type</span></div>
          <div className="gi-blood-grid">{bloodTypes.map((type) => (<button key={type} className={`gi-blood-btn ${selectedBlood === type ? 'active' : ''}`} onClick={() => setSelectedBlood(type)}>{type}</button>))}</div>
        </div>
